...
export default function Login() {
  ...
  const inpFocus = {
    ...
    boxShadow: '0 0 0 3px rgba(129,140,248,.1)'
  }
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, system-ui, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ 
        ...
        <button 
          onClick={() => nav('/student')} 
          style={btnGuest}
          onMouseEnter={e => (e.target.style.background = 'rgba(255,255,255,.12)', e.target.style.borderColor = 'rgba(255,255,255,.2)')}
          onMouseLeave={e => (e.target.style.background = 'rgba(255,255,255,.08)', e.target.style.borderColor = 'rgba(255,255,255,.12)')}
        >
          Continue as Guest
        </button>
      </div>
